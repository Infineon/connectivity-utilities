...
cy_nw_ip_status_change_callback_t ip_status_change_callback = {
    .arg = NULL,
    .cb_func = NULL,
    .priv = NULL
};

void nw_ip_status_change_handler ( cy_nw_ip_status_change_callback_t *cb );
...
void nw_link_change_handler(struct netif *netif)
{
    nw_ip_status_change_handler(&ip_status_change_callback);
}

void nw_status_change_handler(struct netif *netif)
{
    /* FIXME : dummy call as of now till WCM API's are ready */
}

void nw_callback_init(void)
{
    /* FIXME : these will be changed after WCM API's are ready */
    struct netif *net = cy_lwip_get_interface();
    netif_set_link_callback(net, nw_link_change_handler);
    netif_set_status_callback(net, nw_status_change_handler);
}

/* FIXME: This will be deprecated when WCM API's are supported
 *
 */
void nw_ip_status_change_handler ( cy_nw_ip_status_change_callback_t *cb )
{
...
void cy_nw_ip_initialize_status_change_callback(cy_nw_ip_status_change_callback_t *cb, cy_nw_ip_status_change_callback_func_t *cb_func, void *arg)
{
    ...
    ip_status_change_callback.arg = arg;
    ip_status_change_callback.cb_func = cb_func;
    ip_status_change_callback.priv = iface;
    nw_callback_init();
}
